  ...
  return (
    <>
       ...
       <ol className="list-unstyled ms-3 d-flex">
         <li className="text_decoration-none"><Link to="/"><FaHome /></Link></li>
          ...
          <tbody>
            {products && products.map(product => (
              <tr key={product.id}>
                 ...
                 <td>{product.avaible ? <BsCheckCircleFill className="text-success fs-4"/> : <BsXCircleFill className="text-danger fs-4"/>}</td>
                 <td><Button className="m-1" variant="danger" onClick={() => {
                   if(window.confirm('êtes vous sûr ?')){
                     del(product.id)
                   }
                 }}><FaTrash/></Button><Link className="m-1"  state={{ Produit: product, id:product.id}} to={`/ModifProduct`}><Button variant="warning"><FaEdit /></Button></Link></td> 
              </tr>
            ))}
          </tbody>
        </Table>
